...
public class PizzaController : ControllerBase
{
    ...
    public async Task<IActionResult> Post(PizzaDto pizza)
    {
        try
        {
            var retorno = await _service.Add(pizza);

            if (retorno == null) return BadRequest("Você esta tentando adicionar uma pizza já existente");
    ...
    [HttpPut]
    public async Task<IActionResult> Put(PizzaDto pizza)
    {
        try
        {
            var retorno = await _service.Update(pizza);
